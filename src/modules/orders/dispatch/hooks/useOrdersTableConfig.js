...
export const useOrdersTableConfig = ({ resolvedMenuID, filterState }) => {
  ...
  const columnOptions = { skipFields: ordersModuleSchema.skipFields, columnMappings: ordersModuleSchema.columnMappings, tableCellConfig: ordersModuleSchema.tableCellConfig };
  ...
  const resolvedFilterFields = useMemo(() => buildFilterFieldsFromStructure(fields, ordersModuleSchema.defaultColumns.map((key) => ({ label: ordersFallbackColumns.find((column) => column.key === key)?.label || key, value: key, type: "text" })), columnOptions), [fields]);
