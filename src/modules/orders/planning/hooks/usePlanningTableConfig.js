...
export const useOrderPlanningTableConfig = ({ resolvedMenuID, filterState }) => {
    ...
    // );
    const resolvedColumns = useMemo(() => {
        const dbColumns = buildTableColumnsFromStructure(
            fields,
            ordersFallbackColumns,
            columnOptions
        );

        const existingKeys = new Set(dbColumns.map((col) => col.key));

        const extraColumns = (ordersModuleSchema.externalColumns || [])
            .filter((col) => !existingKeys.has(col.key));

        return [...dbColumns, ...extraColumns];
    }, [fields]);
